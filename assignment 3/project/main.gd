...
@onready var flock_manager: FlockManager = $FlockManager
...
func _ready():
	...
	print("Main scene ready! Enhanced sprite loaded with flocking boids!")

func _process(delta):
	# Update flock target to follow the DVD logo
	if flock_manager and enhanced_sprite:
		flock_manager.target_position = enhanced_sprite.position
...
func update_info_display():
	...
	var info_text = "DVD Logo + Flocking Boids\n"
	info_text += "Boids chase the DVD logo!\n"
	info_text += "Boid Count: " + str(flock_manager.boid_count) + "\n"
	info_text += "Logo Speed: " + str(enhanced_sprite.bounce_speed)
